...
export function getJsearchApiKeys() {
  return [process.env.JSEARCH_API_KEY, process.env.JSEARCH_API_KEY_2].filter(Boolean);
}
...
function shouldTryNextJsearchKey(result) {
  ...
  return (
    ...
    result.status === 429 ||
    (result.status === 403 && isJsearchQuotaError(result.status, result.error || ''))
  );
}
...
async function searchJsearchWithKey(apiKey, params) {
  ...
  if (!response.ok) {
    return {
      ok: false,
      quotaExceeded: isJsearchQuotaError(response.status, message),
...
export async function searchJsearch(params) {
  ...
  for (const apiKey of keys) {
    const result = await searchJsearchWithKey(apiKey, params);
    if (result.ok) return result;
    lastResult = result;
    if (!shouldTryNextJsearchKey(result)) break;
  }
...
async function getJsearchJobDetailsWithKey(apiKey, externalId) {
  ...
  if (!response.ok) {
    ...
    return {
      ok: false,
      quotaExceeded: isJsearchQuotaError(response.status, text),
...
export async function getJsearchJobDetails(externalId) {
  ...
  let lastResult = null;
  for (const apiKey of keys) {
    const result = await getJsearchJobDetailsWithKey(apiKey, externalId);
    if (result.ok) return result;
    lastResult = result;
    if (!shouldTryNextJsearchKey(result)) break;
  }
