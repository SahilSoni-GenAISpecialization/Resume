import { JobProvider, PAGE_SIZE, encodeJobId } from './types';
...
function mapJob(job) {
  ...
  const plainDescription = formatJobDescriptionText(description);
  return {
...
export async function getFreehireJobDetails(externalId) {
  ...
  return {
    ...
    job: {
      ...
      location: job.location || '',
      type: job.enrichment?.employment_type || '',
      description: formatJobDescriptionText(description) || description,
      applyUrl: job.url || '',
      postedAt: job.posted_at || job.created_at || '',
      source: job.source || 'Freehire',
      provider: JobProvider.FREEHIRE,
